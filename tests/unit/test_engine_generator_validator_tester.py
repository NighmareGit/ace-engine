"""Unit tests for engine/generator.py, engine/validator.py, and engine/tester.py.

Tests code generation token capture, 4-stage validation (AST/syntax/imports/execution),
and pytest exit-code-5 handling — all with mock transport, no GPU, no inference.
"""
...
def _make_context():
    ...
    ctx = MagicMock()
    ctx.file_tree = {"root": "/tmp/project"}
    ctx.relevant_files = []
    ctx.imports = {}
    ctx.framework = None
    return ctx
...
@patch("engine.prompts.compile_prompt", return_value="compiled prompt")
@patch("engine.prompts.build_project_context", return_value={})
@patch("engine.prompts.build_prd_result", return_value={})
def test_generate_code_captures_total_tokens_from_flat_response(_prd, _proj, _compile):
...
@patch("engine.prompts.build_project_context", return_value={})
@patch("engine.prompts.build_prd_result", return_value={})
def test_generate_code_zero_tokens_when_transport_returns_no_tokens(_prd, _proj, _compile):
...
@patch("engine.prompts.build_project_context", return_value={})
@patch("engine.prompts.build_prd_result", return_value={})
def test_generate_code_extracts_code_from_markdown_fences(_prd, _proj, _compile):
